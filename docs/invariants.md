...
### 1. Локальные данные шифруются at rest — включая метаданные

**С:** 2026-08-12 · **Подробно:** [persistence-encryption.md](persistence-encryption.md)

Всё, что приложение кладёт на диск (IndexedDB, OPFS, localStorage), должно быть
зашифровано ключом, производным от секретов аккаунта. Не только полезная
нагрузка — сквозное шифрование контента (`content_b64`, `value_b64`) было и
раньше, — а весь **конверт**: `user_hash`, `dialog_hash`, `sender_hash`,
`owner_timestamp`, подписи, имена контактов.

**Почему.** Конверт — это социальный граф и хронометраж переписки: кто с кем,
когда, как часто. Для мессенджера, обещающего приватность на устройстве без
интернета, открытый конверт в браузерном профиле — утечка того же порядка, что
и открытый текст сообщений.

**Следствия.**
- Любое новое локальное хранилище проходит через `secureStore` или аналог, а
  не пишет в IndexedDB напрямую.
- Отступления возможны, но **явно**: комментарий у кода, запись в
  persistence-encryption.md, согласование с владельцем. Пример согласованного
  отступления — выключенная флагом персистентность шейпов (L1), потому что
  wa-sqlite не даёт шифрующего VFS.
- Библиотека, которая пишет на диск сама (адаптер, кэш, воркер), проверяется
  на этот пункт **до** внедрения, а не после.
...
### 8. Receipt «прочитано» необратим и отправляется только явным действием

**С:** 2026-08-11 (продуктовое решение)

`dialog_message_receipts` не имеет `deleted_flag` — это append-only факт,
привязанный к ревизии сообщения. Поэтому он **никогда** не создаётся как
побочный эффект показа сообщения на экране, скролла или открытия диалога —
только по кнопке. Бэкенд ограничивает тип значениями `["delivered", "read"]`;
различие «увидел» / «осознанно подтвердил» пока держится только фронтендом.
