...
## 4. Шифрующий VFS для персистентности коллекций (L1)

L1 выключен флагом, потому что wa-sqlite пишет OPFS открытым текстом.
Подробности и обоснование — в [persistence-encryption.md](persistence-encryption.md).
`vfsName` — документированная точка расширения. Работа крупная и
крипто-критичная.
