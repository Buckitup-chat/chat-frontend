...
## 4. Включение персистентности коллекций (L1)

Слой реализован на официальном стеке (`@tanstack/browser-db-sqlite-persistence`,
wa-sqlite поверх OPFS) и выключен флагом в `src/lib/data/persistence.ts`.
Открытый текст в OPFS допустим (см. [persistence-encryption.md](persistence-encryption.md)),
так что включение — решение о производительности, а не о безопасности.

**Что даёт.** Тёплый старт — коллекции поднимаются из SQLite до ответа сети,
на Pi без аплинка история видна сразу. Докачка дельты — Electric продолжает
стрим с сохранённого оффсета вместо полной перекачки шейпа на каждой загрузке.

**Что проверить перед включением по умолчанию.** На живом стейджинге: логин →
сообщения → перезагрузка → мгновенный рендер из SQLite → докачка только
новых строк. Поддержка OPFS в целевых браузерах (Safari, WebView на Pi).
Поведение при двух вкладках через `BrowserCollectionCoordinator`.

---

## 5. Покрытие миграции старых записей в CI
...
## 6. Скиллы Claude Code для повторяющихся процедур
