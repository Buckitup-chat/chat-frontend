...
Слоты адресуются фиксированными UUID, одинаковыми у всех аккаунтов, а чтение
`user_storage` публичное — значит по чужому `user_hash` видно, что у него есть
профиль и когда он правился. Плюс это магические константы без реестра.

Спецификация реализации: [task-user-storage-slot-ids.md](task-user-storage-slot-ids.md).
Замечание общее для обеих веток миграции; схему нужно согласовать.
