...
- Удалить `STORAGE_SLOTS` и `LEGACY_LABELS`.
...
Ленивая, по образцу миграции локального KV; выполняется при первом чтении
профиля после логина.

1. Вычислить корневой uuid, прочитать запись. Есть — работаем по карте, конец.
2. Записи нет — прочитать легаси-адреса `…0001` и `…0002`.
3. Если легаси-профиль найден: сгенерировать случайный uuid для `contacts`,
   записать `contacts` по новому адресу, затем записать корневую запись с
   картой (порядок важен: карта не должна ссылаться на несуществующую строку).
4. Пометить легаси-строки удалёнными (`deleted_flag = true`, пустое значение).
5. Если не найдено ничего — это новый аккаунт, просто создать корневую запись.

Порядок операций между шагами 3 и 4 обязан пройти через барьер
`sendMutationsAndAwaitShape`: следующая запись читает шейп как базу.

## Что миграция не чинит

Старые строки уже опубликованы под предсказуемыми адресами, и `deleted_flag`
их не прячет — тот, кто их уже синхронизировал, сохранил копию, а на сервере
остаётся сама строка-надгробие с историей `owner_timestamp`. Миграция снижает
будущую утечку, а не отменяет прошлую. Полное сокрытие возможно только жёстким
удалением на сервере — это отдельный запрос к бэкенд-команде и, скорее всего,
не стоит усилий для тестовых аккаунтов.
...
- **Карта повреждена или ссылается на отсутствующую строку.** Не создавать
  молча новый слот поверх — это потеря контактов. Сообщить об ошибке и
...
3. Корневой uuid проходит проверку формата `Ecto.UUID` (версия 8, вариант
   RFC 4122).
4. Миграция: легаси-профиль и легаси-контакты переезжают, карта корректна,
   легаси-строки помечены удалёнными.
5. Миграция идемпотентна — повторный запуск не создаёт вторых копий.
6. Слот, отсутствующий в карте, создаётся один раз при двух параллельных
   обращениях.
7. Шейп `user_storage` запрашивается с фильтром по своему `user_hash`.
