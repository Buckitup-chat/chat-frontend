...
-- ==========================================
-- VIEW: user_storage_latest (compatibility)
-- ==========================================
...
SELECT DISTINCT ON (user_hash, uuid)
    ...
    parent_sign_hash, sign_hash, owner_timestamp, sign_b64, updated_at
FROM user_storage
...
--
CREATE OR REPLACE FUNCTION merge_user_cards()
RETURNS TRIGGER AS $$
BEGIN
    IF current_setting('electric.syncing', true) = 'true' THEN
        UPDATE user_cards SET
            sign_pkey = NEW.sign_pkey, crypt_pkey = NEW.crypt_pkey,
            crypt_cert = NEW.crypt_cert, contact_pkey = NEW.contact_pkey,
            contact_cert = NEW.contact_cert, name = NEW.name,
            deleted_flag = NEW.deleted_flag, owner_timestamp = NEW.owner_timestamp,
            sign_b64 = NEW.sign_b64, updated_at = NOW()
        WHERE user_hash = NEW.user_hash AND modified_columns IS NULL;
        IF FOUND THEN RETURN NULL; END IF;
        IF EXISTS (SELECT 1 FROM user_cards WHERE user_hash = NEW.user_hash) THEN
            RETURN NULL;
        END IF;
    END IF;
    RETURN NEW;
...
DROP TRIGGER IF EXISTS trg_merge_user_cards ON user_cards;
CREATE TRIGGER trg_merge_user_cards
BEFORE INSERT ON user_cards
FOR EACH ROW EXECUTE FUNCTION merge_user_cards();

CREATE OR REPLACE FUNCTION merge_user_storage()
RETURNS TRIGGER AS $$
BEGIN
    IF current_setting('electric.syncing', true) = 'true' THEN
        UPDATE user_storage SET
            value_b64 = NEW.value_b64, hash_b64 = NEW.hash_b64,
            deleted_flag = NEW.deleted_flag, parent_sign_hash = NEW.parent_sign_hash,
            sign_hash = NEW.sign_hash, owner_timestamp = NEW.owner_timestamp,
            sign_b64 = NEW.sign_b64, updated_at = NOW()
        WHERE user_hash = NEW.user_hash AND uuid = NEW.uuid AND modified_columns IS NULL;
        IF FOUND THEN RETURN NULL; END IF;
        IF EXISTS (SELECT 1 FROM user_storage WHERE user_hash = NEW.user_hash AND uuid = NEW.uuid) THEN
            RETURN NULL;
        END IF;
    END IF;
    RETURN NEW;
...
DROP TRIGGER IF EXISTS trg_merge_user_storage ON user_storage;
CREATE TRIGGER trg_merge_user_storage
BEFORE INSERT ON user_storage
FOR EACH ROW EXECUTE FUNCTION merge_user_storage();
